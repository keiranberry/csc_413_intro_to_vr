...
public class Navigation : MonoBehaviour
{
  ...
  public void OnGUI()
  {
    ...
    //set our single pixel to some transparent black
    overlay.SetPixel(0, 0, new Color(0, 0, 0, overlayAlpha));
    overlay.Apply();

    //stretch our image over the screen
  ...
  private void OnMoving()
  {
    ...
    float percent = startTime / teleportTime;
    if (percent < 0.5f)
    {
      overlayAlpha = percent * 2;
    }
    else
    {
      if (!halfDone)
      ...
      overlayAlpha = 2 - percent * 2; //OR 1-(percent*2 -1)
    }
